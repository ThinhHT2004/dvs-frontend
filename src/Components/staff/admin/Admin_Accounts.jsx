...
const Admin_Accounts = () => {
    ...
    const getStaffs = async () => {
        try {
            ...
            const staffWithRoles = await Promise.all(staffData.map(async (staff) => {
                const roleResp = await protectedApi.get(`/accounts/${staff.id}`);
                const roleData = roleResp.data;
                return { ...staff, role: roleData.role };
            }));
            const fillteredStaffs = staffWithRoles.filter((staff) => staff.role !== "ADMIN");
                    ...
                    <Table>
                        <TableBody>
                            <TableRow>
                                ...
                                <TableCell sx={{ fontSize: 20, width: 200, color: '#69CEE2' }} align='center'>Role</TableCell>
                                <TableCell sx={{ fontSize: 20, width: 300, color: '#69CEE2' }}>Name</TableCell>
                                ...
                                <Fragment key={staff.id || index}>
                                    <TableRow>
                                        ...
                                        <TableCell align='center'>
                                            <Button
                                                onClick={() => handleViewEditClick(staff)}
                                            >
                                                View & Edit
                                            </Button>
                                        </TableCell>
            ...
            >
                ...
                <Box padding={1}>
                    <TextField
                        ...
                        fullWidth
                        required
                        onChange={(e) => handleDialogChange("phoneNumber", e.target.value)}
                    />
                </Box>
                </DialogContent>
                <DialogActions>
